...
public class SimpleHelloControllerTest extends AbstractDispatcherServletTest {
	...
	public void helloControllerWithAssertMethods() throws ServletException, IOException {
		this.setRelativeLocations("spring-servlet.xml").setClasses(HelloSpring.class)
			.initRequest("/hello", RequestMethod.GET).addParameter("name", "Spring").runService()
			.assertModel("message", "Hello Spring").assertViewName("/WEB-INF/view/hello.jsp");
	}
	
	@Test
	public void helloControllerWithServletPath() throws ServletException, IOException {
		this.setRelativeLocations("spring-servlet.xml").setClasses(HelloSpring.class)
			.setServletPath("/appJcom")
			.initRequest("/appJcom/hello", RequestMethod.GET)
			.addParameter("name", "Spring")
			.runService()
